        ...
        if st.session_state['sit__used_settings']['plotting']['save_heatmap_data']:
            ...
            if os.path.exists(heatmap_data_dir):
                ...
                if not csv_files:
                    ...
                    # Function to create zip file in output directory
                    def create_zip_file():
                        import zipfile
                    ...
                    # Create button to generate zip file
                    if st.button(f'Create heatmap data zip file ({len(csv_files)} files)'):
                        zip_path = create_zip_file()
                        st.success(f'Zip file created at: {zip_path}')
            else:
