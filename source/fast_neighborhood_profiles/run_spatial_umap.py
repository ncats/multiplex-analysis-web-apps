...
def main():
    ...
    st.session_state.setdefault(key, False)
    set_cpu_pool_size = st.checkbox("Set CPU pool size", key=key, help="If unchecked, algorithm will use all available CPUs.")

    # Set number of threads to use.
    key = ST_KEY_PREFIX + "cpu_pool_size"
    st.session_state.setdefault(key, 4)
    cpu_pool_size = st.number_input("CPU pool size:", min_value=1, step=1, key=key, disabled=not st.session_state[ST_KEY_PREFIX + "set_cpu_pool_size"])
    ...
    fnp_main.save_pandas_df_to_file(spatial_umap.cells, handle="sumap_cells", file_format="parquet")
