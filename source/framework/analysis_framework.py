...
def job_submission(job_name, inputs, analysis_purpose, st_key_prefix, preprocess={}):
    ...
    with st.columns(1, border=True)[0]:
        ...
        if st.button(f"Run {analysis_purpose}", type=("primary" if do_async_analysis else "secondary")):
            if preprocess:
                pre_func = preprocess["function"]
                pre_args = preprocess["args"]
                result_dict = pre_func(**pre_args)
